import numpy as np
import os
from envs.utils.rendering import fill_coords, point_in_rect, point_in_line, point_in_circle
...
def generate_dir(env_name):
    os.makedirs("envfiles/" + env_name, exist_ok=True)
    return "envfiles/" + env_name + "/" + env_name
...
def create_door(tile_size, status, color=agent_colors[1]):
